...
def loginAdmin():
    if request.method == 'POST':
        ...
        query = ("SELECT IdUsuario,Nombres FROM usuarios WHERE Usuario = %s AND Password = %s AND IdRol = 1 AND IdEstado =1")
...
def publicarDocumento(iduser):
    ...
    if request.method == 'POST':
        ...
        query = ("INSERT INTO publicaciones (IdUsuario,Fecha,IdTipo,Autor,Proyecto,Ruta)VALUE(%s,%s,%s,%s,%s,%s)")
        valores = (idu,Fecha,IdTipo,Autor,Proyecto,Ruta)
        cur.execute(query, valores)
        cur.connection.commit()
        cur.close()
...
def listarArticulosAdmin():
    cur = conn.cursor()
    query = ("SELECT IdPublicacion,Fecha,Autor,Proyecto,Ruta FROM publicaciones WHERE IdTipo =1 ORDER BY Fecha DESC")
    cur.execute(query)
    result = cur.fetchall()
    cur.close()
    return result

def listarPublicados():
    cur = conn.cursor()
    query = ("SELECT P.IdPublicacion,P.Fecha,P.Autor,P.Proyecto,P.Ruta,TP.Tipo FROM publicaciones P INNER JOIN tipo_publicacion TP ON (P.IdTipo=TP.IdTipo) ORDER BY P.Fecha DESC")
    cur.execute(query)
    result = cur.fetchall()
    cur.close()
    return result
...
def listarPublicacionesAdmin():
    cur = conn.cursor()
    query = ("SELECT P.IdPublicacion,P.Fecha,P.Autor,P.Proyecto,P.Ruta,TP.Tipo FROM publicaciones P INNER JOIN tipo_publicacion TP ON (P.IdTipo=TP.IdTipo) WHERE P.IdTipo =2 ORDER BY P.Fecha DESC")
    cur.execute(query)
    result = cur.fetchall()
    cur.close()
    return result
...
def getMRolUsuario():
    if request.method == 'POST':
        ...
        query = """UPDATE usuarios SET IdRol = %s WHERE IdUsuario = %s"""
        valores = (IdRol,IdUsuario)
        cur.execute(query, valores)
        cur.connection.commit()
        cur.close()
        filas_afectadas = cur.rowcount
...
def guardarCorreo():
    if request.method == 'POST':
        ...
        query = "INSERT INTO config_correo (Servidor,Puerto,Remitente,Password,Asunto,Mensaje)VALUE(%s,%s,%s,%s,%s,%s)"
        valores = (Servidor,Puerto,Remitente,Password,Asunto,Mensaje)
        print(Servidor,Puerto,Remitente,Password,Asunto,Mensaje)
        cur.execute(query, valores)
        cur.connection.commit()
        cur.close()
        filas_afectadas = cur.rowcount
